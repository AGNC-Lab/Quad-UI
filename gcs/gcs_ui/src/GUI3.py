...
class Curve():
  def __init__(self, color, name):
    ...
    self.dataY = []
    self.dataX = []
    self.curve1 = pg.PlotCurveItem(self.dataX, self.dataY, pen= color, name = name)
  ...
  def update(self):
    self.curve1.setData(self.dataX, self.dataY)
...
class RosPlot():
  def __init__(self, num, name):
    ...
    for i in range(num):
      ...
      #When adding new topics and data... add to this box
      if name == topics[0]:
        offset = 0
      if name == topics[1]:
        offset = 1
      #Stop Editing Here
      curve = Curve(colors[i + offset],datas[offset + i]) 
...
#
#
#EDIT THIS (TOP) (I CAN REMOVE THIS ONE)
#
#
#
#
#
    if self.topic == "/counter":
       self.plotWidget = RosPlot(1, topics[0])
    elif self.topic == "/Quad_RPY":
       self.plotWidget = RosPlot(3, topics[1])
#
#
#
#EDIT THIS (BOTTOM)
...
#      
  ...
  def getCurve(self, number):
    return self.plotWidget.getCurve(number)
...
#
      ...
      if not item.checkState(): 
        ...
        self.plots[self.currentPlot - 1].getPlot().clear()  
        self.plots[self.currentPlot - 1].getPlot().plotItem.legend.items = [] 

        print self.models[self.currentPlot - 1].rowCount()
        for row in range(self.models[self.currentPlot - 1].rowCount()):
          item = self.models[self.currentPlot - 1].item(row)
